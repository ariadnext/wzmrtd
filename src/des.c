 ...
 * Parity bits are not checked
 */
void TDES_Init(TDES_CTX_ST *ctx, const BYTE key1_data[8], const BYTE key2_data[8], const BYTE key3_data[8])
{
  DES_Init(&ctx->key1_ctx, key1_data);
  DES_Init(&ctx->key2_ctx, key2_data);
  if (key3_data != NULL)
  {
    DES_Init(&ctx->key3_ctx, key3_data);
  } else
  {
    memcpy(&ctx->key3_ctx, &ctx->key1_ctx, sizeof(DES_CTX_ST));
  }
}
 ...
 */
void TDES_Encrypt(TDES_CTX_ST *ctx, BYTE data[8])
{
  DES_core(&ctx->key1_ctx, data, TRUE);
  DES_core(&ctx->key2_ctx, data, FALSE);
  DES_core(&ctx->key3_ctx, data, TRUE);
}
 ...
 */
void TDES_Decrypt(TDES_CTX_ST *ctx, BYTE data[8])
{
  DES_core(&ctx->key3_ctx, data, FALSE);
  DES_core(&ctx->key2_ctx, data, TRUE);
  DES_core(&ctx->key1_ctx, data, FALSE);
}
